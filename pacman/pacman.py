...
start_img = pygame.image.load('assets/menu_images/start.png').convert_alpha()
exit_img = pygame.image.load('assets/menu_images/exit.png').convert_alpha()
background_img = pygame.image.load('assets/menu_images/background.png').convert_alpha()
player_x=432
player_y=590
direction=0
blinky_x=56
...
def draw_misc():
    ...
    screen.blit(score_text, (15, 820))
    for i in range(lives):
        screen.blit(pygame.transform.scale(player_images[0], (25,25)), (780 + i * 40, 817))
    if game_over:
...
def create_table():
    cursor.execute("CREATE TABLE IF NOT EXISTS ScoreTable (Skor INT,Tarih TEXT)")
    connection.commit()


def add_score():
    connection.execute("INSERT INTO ScoreTable (Skor,Tarih) VALUES (?,?)",(score,date))
    connection.commit()


def select_score():
    topScore=()
    cursor.execute("SELECT Skor FROM ScoreTable")
    data=cursor.fetchall()
    for i in data:
        if i > topScore:
            topScore=i
    top = font.render(f'Top Score: {topScore[0]}', True, 'yellow')
    screen.blit(top, (15, 820))
